...
export default function AiChat() {
  ...
  const fetchWebhookFromSupabase = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('link_ref').select('*').eq('topic', 'webhook chatbot').single();
      if (error) {
  ...
  const saveWebhookConfig = async (newUrl: string) => {
    try {
      const {
        error
      } = await supabase.from('link_ref').update({
        linked: newUrl,
      ...
      <div className="bg-card rounded-lg shadow-soft overflow-hidden">
          ...
          <div className="flex items-center justify-between">
            ...
            <Dialog open={isSettingsOpen} onOpenChange={setIsSettingsOpen}>
              <DialogTrigger asChild>
                <Button variant="ghost" size="sm" className="text-pink-600 dark:text-pink-400 hover:bg-pink-200 dark:hover:bg-pink-800/50">
              ...
              <DialogContent className="sm:max-w-md">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <Input placeholder="https://your-webhook-url.com/chat" defaultValue={webhookUrl} onKeyPress={e => {
                    if (e.key === 'Enter') {
        ...
        <ScrollArea className="h-[400px] sm:h-[500px] p-4">
          <div className="space-y-4">
            {messages.map(message => <div key={message.id} className={`flex gap-3 ${message.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                {message.sender === 'bot' && <div className="w-8 h-8 bg-pink-100 dark:bg-pink-900/50 rounded-full flex items-center justify-center flex-shrink-0 mt-1">
                    <Bot className="w-4 h-4 text-pink-600 dark:text-pink-400" />
                  </div>}
                
                <div className={`max-w-[80%] sm:max-w-[70%] px-4 py-3 rounded-lg ${message.sender === 'user' ? 'bg-pink-500 text-white' : 'bg-pink-50 dark:bg-pink-950/50 text-foreground border border-pink-200 dark:border-pink-800'}`}>
                  <p className="text-sm leading-relaxed">{message.content}</p>
                  <span className={`text-xs mt-1 block ${message.sender === 'user' ? 'text-pink-100' : 'text-pink-600 dark:text-pink-400'}`}>
                ...
                {message.sender === 'user' && <div className="w-8 h-8 bg-pink-500 rounded-full flex items-center justify-center flex-shrink-0 mt-1">
            ...
            {isLoading && <div className="flex gap-3 justify-start">
                ...
                <div className="bg-pink-50 dark:bg-pink-950/50 border border-pink-200 dark:border-pink-800 px-4 py-3 rounded-lg">
                  <div className="flex space-x-1">
                    ...
                    <div className="w-2 h-2 bg-pink-400 rounded-full animate-bounce" style={{
                  animationDelay: '0.1s'
                }}></div>
                    <div className="w-2 h-2 bg-pink-400 rounded-full animate-bounce" style={{
                  animationDelay: '0.2s'
        ...
        <div className="border-t border-pink-200 dark:border-pink-800/50 p-4 bg-pink-50/30 dark:bg-pink-950/20">
          <div className="flex gap-2 sm:gap-3">
            <Input value={inputValue} onChange={e => setInputValue(e.target.value)} onKeyPress={handleKeyPress} placeholder="พิมพ์ข้อความของคุณที่นี่..." className="flex-1 border-pink-200 dark:border-pink-800 focus-visible:ring-pink-400 focus-visible:border-pink-400" disabled={isLoading} />
            <Button onClick={sendMessage} disabled={!inputValue.trim() || isLoading} className="bg-pink-500 hover:bg-pink-600 text-white flex-shrink-0 px-3 sm:px-4" size="default">
